        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
            ...
            <div className="relative w-full lg:w-[450px]">
              <Search className={`absolute left-5 top-1/2 -translate-y-1/2 h-6 w-6 transition-colors ${search ? 'text-indigo-600' : 'text-gray-400'}`} />
              <input
                type="text"
          ...
          <div className="mt-4 overflow-x-auto pb-2 -mx-6 px-6 no-scrollbar">
            <div className="flex items-center gap-3 min-w-max">
               <div className="flex items-center gap-2 px-3 py-1 bg-gray-100 dark:bg-zinc-800 rounded-lg flex-shrink-0">
                  <span className="font-black text-[10px] text-gray-500 uppercase tracking-widest">{t('course.level')}</span>
               </div>
               {levels.map(lvl => (
                 <button 
                   key={lvl}
        ...
        {loading && (!courses || !courses.length) ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
           ...
           <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/50 p-12 rounded-[40px] text-center max-w-2xl mx-auto">
             ...
             <h2 className="text-3xl font-black text-gray-900 dark:text-white mb-4">{t('course.connection_interrupted')}</h2>
             <p className="text-gray-600 dark:text-gray-400 font-medium mb-8">{error}</p>
             <button onClick={() => window.location.reload()} className="px-8 py-4 bg-red-600 text-white font-black rounded-2xl hover:bg-red-700 transition-all shadow-xl shadow-red-600/20" aria-label="Retry connection">
               {t('course.retry')}
             </button>
           </div>
          ...
          <div className="text-center py-32 bg-white dark:bg-zinc-900 rounded-[48px] border-2 border-dashed border-gray-100 dark:border-zinc-800">
            ...
            <h2 className="text-3xl font-black text-gray-900 dark:text-white mb-2">{t('course.no_matches')}</h2>
            <p className="text-gray-500 font-bold">{t('course.adjust_filters')}</p>
            <button 
