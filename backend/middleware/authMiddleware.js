...
const admin = (req, res, next) => {
  ...
  if (req.user && (req.user.role === 'admin' || req.user.role === 'superAdmin') && req.user.status === 'approved') {
...
const superAdmin = (req, res, next) => {
  ...
  if (req.user && req.user.role === 'superAdmin' && req.user.status === 'approved') {
...
const cashManager = (req, res, next) => {

  if (req.user && req.user.role === 'cashManager' && req.user.status === 'approved') {

    next();

  } else {

    res.status(403).json({ message: 'Access denied. Cash Manager role required.' });
...
export { protect, optionalProtect, admin, superAdmin, cashManager, instructor, student, adminOrInstructor, anyRole };
